...
mod deadlock_regression_tests {
    ...
    fn test_scoped_sender_cleanup_prevents_deadlock() {
        ...
        // CRITICAL: Use scoped sender_clone like in the fix
        {
            ...
            chunks.into_par_iter().try_for_each_init(
                || sender_clone.clone(),
        ...
        assert_eq!(processed.load(Ordering::Relaxed), num_chunks, "All chunks should be processed");
        
        println!("✓ Deadlock prevention test passed: {} chunks processed with capacity {}", 
    ...
    fn test_channel_capacity_stress() {
        ...
        for capacity in [1, 2, 4, 8, 16] {
            ...
            assert_eq!(received.load(Ordering::Relaxed), num_chunks,
                      "Capacity {} should handle {} chunks", capacity, num_chunks);
        }

...
impl ReferenceGenome {
    ...
    pub fn new_with_cache_size(fasta_path: &str, cache_size: usize) -> Result<Self> {
        ...
        if !std::path::Path::new(fasta_path).exists() {
            return Err(RedicatError::FileNotFound(
                format!("FASTA file not found: {}", fasta_path)
            ));
        }
        
        if !std::path::Path::new(&index_path).exists() {
            return Err(RedicatError::FileNotFound(
                format!("FASTA index file not found: {}. Please create it using: samtools faidx {}", 
                       index_path, fasta_path)
            ));
        }
        
        let index = Index::from_file(&index_path)
            .map_err(|e| RedicatError::ReferenceGenome(
                format!("Failed to load FASTA index: {:?}", e)
            ))?;
        
        let reader = IndexedReader::with_index(
            std::fs::File::open(fasta_path)
                .map_err(|e| RedicatError::FileNotFound(format!("Cannot open FASTA file: {}", e)))?,
            index
        );
        
        let sequences = Self::read_sequence_names(&index_path)?;
        
        info!("Loaded reference genome with {} sequences, cache size: {}", 
              sequences.len(), cache_size);
    ...
    pub fn get_multiple_refs_chunked(&self, positions: &[String], chunk_size: usize) -> Result<Vec<char>> {
        if positions.is_empty() {
            return Ok(Vec::new());
        }
    ...
    pub fn preload_positions(&self, positions: &[String]) -> Result<()> {
        ...
        info!("Preloading complete. Cache size: {}, usage: {:.1}%", cache_size, usage);
    ...
    fn read_sequence_names(index_path: &str) -> Result<Vec<String>> {
        ...
        if sequences.is_empty() {
            return Err(RedicatError::EmptyData("No sequences found in FASTA index".to_string()));
        }
...
mod tests {
    ...
    fn test_position_validation() {
        ...
        assert_eq!("chr1:1000".split(':').collect::<Vec<_>>(), vec!["chr1", "1000"]);

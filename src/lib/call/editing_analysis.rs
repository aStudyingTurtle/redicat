...
fn calculate_observation_sums_vectorized(mut adata: AnnDataContainer) -> Result<AnnDataContainer> {
    ...
    // Add all computed sums to obs DataFrame - Fixed DataFrame mutation
    for (layer_name, sums) in layer_sums {
        let series = Series::new(layer_name.into(), sums);
        adata.obs = adata.obs.with_column(series).unwrap().clone();
    }
...
pub fn calculate_cei(mut adata: AnnDataContainer) -> Result<AnnDataContainer> {
    ...
    adata.obs = adata.obs.with_column(cei_series).unwrap().clone();
    info!("CEI calculated");
    Ok(adata)
}

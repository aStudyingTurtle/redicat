...
fn convert_array_to_csr_f64(array_data: ArrayData) -> Result<CsrMatrix<f64>> {
    match array_data {
        ArrayData::CsrMatrix(dyn_csr) => match dyn_csr {
            DynCsrMatrix::F64(matrix) => Ok(matrix),
        ...
        _ => Err(RedicatError::DataProcessing("Not a CSR matrix".to_string())),
    }
}
...
fn convert_array_to_csr_u32(array_data: ArrayData) -> Result<CsrMatrix<u32>> {
    match array_data {
        ArrayData::CsrMatrix(dyn_csr) => match dyn_csr {
            DynCsrMatrix::U32(matrix) => Ok(matrix),
            DynCsrMatrix::F32(matrix) => {
                let (row_offsets, col_indices, values) = matrix.csr_data();
                let values_u32: Vec<u32> =
                    values.iter().map(|&v| v.round().max(0.0) as u32).collect();
                CsrMatrix::try_from_csr_data(
                    matrix.nrows(),
                    matrix.ncols(),
                    row_offsets.to_vec(),
...
fn estimate_dataframe_size(df: &DataFrame) -> usize {
    ...
    for column in df.get_columns() {
        size += column.len() * std::mem::size_of::<f64>(); // Rough estimate
    }
    size
}

fn estimate_csr_matrix_size_f64(matrix: &CsrMatrix<f64>) -> usize {
    let row_offsets_size = (matrix.nrows() + 1) * std::mem::size_of::<usize>();
    let col_indices_size = matrix.nnz() * std::mem::size_of::<usize>();
    let values_size = matrix.nnz() * std::mem::size_of::<f64>();
    row_offsets_size + col_indices_size + values_size
}

fn estimate_csr_matrix_size_u32(matrix: &CsrMatrix<u32>) -> usize {
    let row_offsets_size = (matrix.nrows() + 1) * std::mem::size_of::<usize>();
    let col_indices_size = matrix.nnz() * std::mem::size_of::<usize>();
    let values_size = matrix.nnz() * std::mem::size_of::<u32>();
    row_offsets_size + col_indices_size + values_size
}
